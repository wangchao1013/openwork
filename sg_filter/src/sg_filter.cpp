...
float_mat::float_mat(const size_t rows, const size_t cols, const double defval)
    ...
    //    char buffer[1024];
        ...
        //printf("cannot build matrix with %d rows and %d columns\n");
    ...
    float_vect& scale, int_vect& idx, double tol)
{
     ...
     * used. */
    double sg_smooth(const float_vect & v, const int width, const int deg)
    {
        double res = 0.0;
...
//#endif    
    ...
    static float_vect lsqr_fprime(const float_vect & b, const uint64_t deg)
    {
        const size_t rows(b.size());
        const size_t cols(deg + 1);
        float_mat A(rows, cols);
        ...
        const int deg, const double h)
    {
        ...
        if ((width < 1) || (deg < 1) || (v.size() < (2 * (uint64_t)width + 2))) {
